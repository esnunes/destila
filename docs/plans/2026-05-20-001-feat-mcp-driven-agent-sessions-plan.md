...
- *Exports FK exclusivity:* inserting a `workflow_session_metadata` row with both FKs set raises (CHECK violation).
...
- Phase handoff:
  1. `SessionServer` receives `{:phase_advanced, new_index}`.
  2. `EmbeddedHost.stop_phase/1` cleanly terminates the current `claude` process via the terminal.
  3. New per-session `.mcp.json` is regenerated (workflow may have phase-specific config in the future).
  4. `EmbeddedHost.start_phase/2` spawns the new `claude` process with the next phase's `agent_command` and `system_prompt`.
  5. Once SSE connects from the new bridge, `SessionServer` transitions to `:active` and pushes the kickoff prompt.
...
| Embedded terminal stdin race during handoff | Medium | Medium — kickoff prompt lost or sent to wrong agent | Sequence handoff strictly: monitor `:DOWN` on the old terminal pid before spawning the new one; queue pushes during the gap. |
