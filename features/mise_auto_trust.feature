Feature: Mise Auto-Trust
  A project may opt into automatic `mise trust` execution inside each
  ...
  `mise trust -y --all -C <worktree path>` before the existing setup
  command runs, trusting the worktree's own `mise.toml` together with
  every parent `mise.toml` up the directory chain so subsequent commands
  can rely on mise-managed tools without prompts. When the flag is off,
  no mise invocation occurs and worktree lifecycle is identical to
  before. Mise failures (missing binary, non-zero exit) are logged at
  warning level server-side and never block worktree readiness or the
  subsequent setup command.
  ...
  Scenario: Worktree preparation runs mise trust before the setup command
    ...
    Then `mise trust -y --all -C <worktree path>` is executed so the worktree config and its parent configs are all trusted
  ...
  Scenario: Worktree preparation skips mise trust when the flag is off
    ...
    When a new workflow session's worktree is ready
    Then `mise trust` is not executed
  ...
  Scenario: A non-zero mise exit is logged and does not block setup
    ...
    And `mise trust` exits with a non-zero status
