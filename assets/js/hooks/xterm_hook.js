...
export function createXtermHook(opts = {}) {
  const {
    interactive = false,
    scrollback = 0,
    forwardResize = false,
  ...
  return {
    mounted() {
      ...
      if (handleClear) {
        this.handleEvent("clear", () => this.term.reset())
      }
...
export const TerminalPanel = createXtermHook({
  interactive: true,
  scrollback: 0,
  forwardResize: true,
  handleExited: true,
...
export const ServiceLogViewer = createXtermHook({
  interactive: false,
  scrollback: 10000,
  handleClear: true,
})
